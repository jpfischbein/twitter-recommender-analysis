...
#all_csvs = os.listdir('/Users/mhilbert/PycharmProjects/Scripts')
driver = webdriver.Chrome('./chromedriver') # To be used for just recommend video page.
second_driver = webdriver.Chrome('./chromedriver') # To be used to open up windows.
third_driver = webdriver.Chrome('./chromedriver') # To be used for alchemy.

# Writes the header in the csv..
def csv_header():
	iofile = open('./yt.csv','a')
...
def youtube_search():
	option = int(raw_input("How many search terms? (Enter 1 for 1 and any other number to go to the csv.) "))

	if option == 1:
		search_term = raw_input("What is your search term? ")
		search_box = driver.find_element_by_xpath('//*[@id="masthead-search-term"]').send_keys(search_term)
		magnifying_glass_button = driver.find_element_by_xpath('//*[@id="search-btn"]').click()
		stall(1)
		watch_video_fill_history()
	else:
		readin_searches()
		for i in range(len(searchterms)):
			search_box = driver.find_element_by_xpath('//*[@id="masthead-search-term"]').send_keys(searchterms[i])
			magnifying_glass_button = driver.find_element_by_xpath('//*[@id="search-btn"]').click()
			stall(2)
			watch_video_fill_history()

def watch_video_fill_history():

	top_20_videos = driver.find_elements_by_css_selector('a.yt-uix-sessionlink.yt-uix-tile-link.yt-ui-ellipsis.yt-ui-ellipsis-2.spf-link')

	# Get the first video's hyperlink address
	hyperlink = top_20_videos[0].get_attribute('href')

	# Open the first video and watch it.
	driver.get(hyperlink)

	# Watch the video for xy seconds so it goes to watch history
	stall(5)


def readin_searches():
	# Fix this absolute path to be congruennt on windows and where the file is.
...
def recommended_scraper():
	...
	# Load More Videos Button. You might need to click this mulitiple times to get more than 100. So put a for loop here if need be.
	...
	# Run through all the xxx (specified number of) videos -- range (xxx) -- and collect their sentiment and emotions.
	for i in range(32):
